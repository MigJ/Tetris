...
** Last update Sun Mar 19 16:24:14 2017 detroy_j
*/
  ...
  while (44)
    {
      ...
      if (game.act.key == opts->k_quit[0])
	break;
      else if (game.act.key == opts->k_pause[0])
	break;
      cbreak();
      nodelay(win.w, TRUE);
...
t_game		key_pressed(t_options *opts, t_window win,
			    t_game game, char ***piece)
{
  if (game.act.key == KEY_LEFT)
    game.co.x = (game.co.x != COL_WALL_LEFT && COL_LEFT != 1)
      ? clear_piece(*piece, game.co.y, game.co.x, win.ground), game.co.x - 1
      : game.co.x;
  else if (game.act.key == KEY_RIGHT)
    game.co.x = (game.co.x != COL_WALL_RIGHT && COL_RIGHT != 1)
      ? clear_piece(*piece, game.co.y, game.co.x, win.ground), game.co.x + 1
      : game.co.x;
  else if (game.act.key == KEY_DOWN)
    game.act.drop = 1;
  else if (game.act.key == opts->k_quit[0])
    return (game);
  else if (game.act.key == opts->k_pause[0])
    {
      while ((game.act.key = getch()) != opts->k_pause[0]);
      game.act.key = 0;
    }
  else if (game.act.key == KEY_UP
